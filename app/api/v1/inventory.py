...
def edit_item(item_id: int, data: ItemUpdate,
...
def stock_movement(data: StockMovementCreate,
                   db: Session = Depends(get_db), current_user=Depends(get_hr)):
...
def export_inventory_excel_route(db: Session = Depends(get_db), current_user=Depends(get_hr)):
    buffer = export_inventory_excel(get_all_items(db))
    return StreamingResponse(buffer,
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={"Content-Disposition": "attachment; filename=inventory_report.xlsx"})

@router.get("/item/{item_id}/barcode")
def item_barcode(item_id: int, db: Session = Depends(get_db),
                 current_user=Depends(get_current_user)):
    """Returns a PNG barcode (EAN-13 or Code128). Print and label the item."""
    return Response(content=generate_item_barcode(db, item_id), media_type="image/png")