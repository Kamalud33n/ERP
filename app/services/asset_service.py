...
def create_asset(db: Session, data: AssetCreate, created_by: int) -> Asset:
    ...
    asset = Asset(
        name=data.name, asset_code=asset_code, category=data.category,
        serial_number=data.serial_number, location=data.location,
        department=data.department, purchase_date=data.purchase_date,
        purchase_price=data.purchase_price, current_value=data.purchase_price,
        depreciation_rate=data.depreciation_rate, description=data.description,
        assigned_to=data.assigned_to, created_by=created_by
    )
    db.add(asset); db.commit(); db.refresh(asset)
    if data.purchase_price and data.purchase_price > 0:
        from app.services.finance_service import post_gl_entry
        post_gl_entry(db, entry_date=data.purchase_date or date.today(),
...
def calculate_depreciation(db: Session, asset_id: int) -> dict:
    ...
    if delta > 0:
        ...
        post_gl_entry(db, entry_date=date.today(), description=f"Depreciation Expense - {asset.asset_code}",
                      account_type="expense", debit=delta, reference=ref, created_by=asset.created_by)
        post_gl_entry(db, entry_date=date.today(), description=f"Accumulated Depreciation - {asset.asset_code}",
                      account_type="asset", credit=delta, reference=ref, created_by=asset.created_by)
    return {
        "asset_id": asset_id, "asset_code": asset.asset_code,
        "purchase_price": asset.purchase_price, "years_used": round(years, 2),
        "depreciated": round(depreciated, 2), "current_value": round(current_value, 2),
...
def get_asset_summary(db: Session) -> dict:
    ...
    return {
        ...
        "total_value":   round(sum(a.current_value for a in assets if a.status == "active"), 2)
    }
...
def create_maintenance(db: Session, data: MaintenanceCreate, done_by: int) -> MaintenanceLog:
    ...
    if data.status == "in_progress":
        ...
        asset.status = "active"
    log = MaintenanceLog(
        asset_id=data.asset_id, type=data.type, description=data.description,
        cost=data.cost, maintenance_date=data.maintenance_date,
        next_due_date=data.next_due_date, status=data.status, done_by=done_by
    )
    db.add(log); db.commit(); db.refresh(log)
    return log

def get_maintenance_by_asset(db: Session, asset_id: int):
    return db.query(MaintenanceLog).filter(MaintenanceLog.asset_id == asset_id).order_by(MaintenanceLog.created_at.desc()).all()
...
# ── QR Code Generation ─────────────────────────────────
def generate_asset_qr(db: Session, asset_id: int) -> bytes:
    """Returns a QR code PNG embedding the full asset details as JSON."""
    import qrcode, io, json
    asset = get_asset_by_id(db, asset_id)
    payload = json.dumps({
        "id": asset.id, "asset_code": asset.asset_code, "name": asset.name,
        "category": asset.category, "serial": asset.serial_number,
        "location": asset.location, "department": asset.department,
        "status": asset.status, "current_value": round(asset.current_value, 2),
    }, ensure_ascii=False)
